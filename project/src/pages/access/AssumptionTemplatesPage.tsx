  ...
  return (
    <div style={{ position: 'fixed', inset: 0, overflow: 'hidden', pointerEvents: 'none', zIndex: 0, opacity: 'var(--grid-opacity)' }}>
      <svg width="100%" height="100%">
        <defs>
          <pattern id="asmTmplGrid" width="48" height="48" patternUnits="userSpaceOnUse">
            <path d="M 48 0 L 0 0 0 48" fill="none" stroke="var(--accent-gold)" strokeWidth="0.4" />
          </pattern>
        </defs>
        <rect width="100%" height="100%" fill="url(#asmTmplGrid)" />
      </svg>
    </div>
  );
}
...
const AssumptionTemplatesPage: React.FC = () => {
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        .asm-tmpl-page { min-height: 100vh; font-family: var(--font-body); background: var(--bg-primary); color: var(--text-primary); overflow-x: hidden; position: relative; }
        .asm-tmpl-page::before { content: ''; position: fixed; inset: 0; background: radial-gradient(ellipse 60% 45% at 25% 10%, var(--overlay-gold), transparent), radial-gradient(ellipse 55% 40% at 75% 90%, var(--overlay-green), transparent); pointer-events: none; z-index: 0; }
        .asm-tmpl-page a { color: inherit; }
        @media (max-width: 768px) { .asm-tmpl-hero-title { font-size: 28px !important; } .asm-tmpl-hero-inner { padding: 36px 16px 48px !important; } }
      `}</style>
      <div className="asm-tmpl-page">
        <PremiumGrid />
        <PremiumParticles />
        <div style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ position: 'relative', overflow: 'hidden', background: 'var(--bg-hero)', borderBottom: '1px solid var(--border-gold)', animation: 'fadeSlideUp 0.7s ease-out both' }}>
            ...
            <div className="asm-tmpl-hero-inner" style={{ maxWidth: 1200, margin: '0 auto', padding: '48px 32px 72px', position: 'relative', zIndex: 1 }}>
              ...
              <h1 className="asm-tmpl-hero-title" style={{ fontFamily: 'var(--font-display)', fontSize: 46, fontWeight: 400, textAlign: 'center', color: '#e8ede2', lineHeight: 1.15, letterSpacing: '-0.02em', marginBottom: 16 }}>
                Loan Assumption <span style={{ fontStyle: 'italic', color: '#cda349' }}>Templates & Docs</span>
              </h1>
              <p style={{ textAlign: 'center', fontSize: 16, color: 'rgba(232,237,226,0.6)', lineHeight: 1.75, maxWidth: 620, margin: '0 auto 0', fontFamily: 'var(--font-body)' }}>
                Professional templates and documents for loan assumption submissions. Full library coming soon.
              </p>
            </div>
          </div>
          ...
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '48px 24px' }}>
            <div style={{ background: 'var(--bg-card)', backdropFilter: 'var(--glass-blur)', WebkitBackdropFilter: 'var(--glass-blur)', borderRadius: 24, border: '1px solid var(--border-gold)', padding: '72px 48px', textAlign: 'center', boxShadow: 'var(--shadow-gold)', animation: 'fadeSlideUp 0.7s ease-out 0.2s both' }}>
              ...
              <p style={{ fontSize: 16, color: 'var(--text-secondary)', lineHeight: 1.75, maxWidth: 560, margin: '0 auto 32px', fontFamily: 'var(--font-body)' }}>
                Our team is preparing a comprehensive library of professional templates for the loan assumption process including assumption letters, cover sheets, and submission tools. Check back soon.
              </p>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'center' }}>
                {['Assumption Request Letter', 'Cover Sheet Templates', 'Submission Checklists', 'Financial Summary Template'].map((item, i) => (
...
export default AssumptionTemplatesPage;
