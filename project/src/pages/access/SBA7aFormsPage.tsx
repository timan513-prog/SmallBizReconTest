...
import { ArrowLeft, FileText, Download, ExternalLink, ChevronDown, ShieldCheck, Sun, Moon, CircleAlert as AlertCircle } from 'lucide-react';
...
const REQUIRED_FORMS: FormItem[] = [
  {
    ...
    description: 'Required for all 7(a) loan applications. Collects borrower background and ownership details.',
    viewUrl: 'https://www.sba.gov/document/sba-form-1919-borrower-information-form',
    downloadPath: 'https://www.sba.gov/document/sba-form-1919-borrower-information-form',
    customDate: '*Verify latest version on SBA.gov*',
    accentColor: '#cf6b6b',
  },
  {
    title: 'SBA Form 1920 — Lender\'s Application for 7(a) Loan Guaranty',
    description: 'The lender\'s application form submitted to SBA for loan guaranty approval.',
    viewUrl: 'https://www.sba.gov/document/sba-form-1920-lenders-application-guaranty',
    downloadPath: 'https://www.sba.gov/document/sba-form-1920-lenders-application-guaranty',
    customDate: '*Verify latest version on SBA.gov*',
    accentColor: '#cf6b6b',
  },
  {
...
const OPTIONAL_FORMS: FormItem[] = [
  {
    ...
    description: 'Profit & loss, balance sheet, and cash flow projections. Your lender will specify exact requirements.',
  ...
  {
    title: 'Business Plan',
    description: 'Required for startups and some existing businesses. Should include market analysis and financial projections.',
    customDate: '*See Templates section for examples*',
    accentColor: '#d4b76a',
  },
];
...
const INFORMATIONAL_FORMS: FormItem[] = [
  ...
  },
  {
    ...
    viewUrl: 'https://www.sba.gov/funding-programs/loans/7a-loans',
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, overflow: 'hidden', pointerEvents: 'none', zIndex: 0, opacity: 'var(--grid-opacity)' }}>
      <svg width="100%" height="100%">
        <defs>
          <pattern id="s7aFormsGrid" width="48" height="48" patternUnits="userSpaceOnUse">
            <path d="M 48 0 L 0 0 0 48" fill="none" stroke="var(--accent-gold)" strokeWidth="0.4" />
          </pattern>
          <pattern id="s7aFormsGridDiag" width="96" height="96" patternUnits="userSpaceOnUse">
            <path d="M 0 96 L 96 0" fill="none" stroke="var(--accent-gold)" strokeWidth="0.2" opacity="0.5" />
          </pattern>
        </defs>
        <rect width="100%" height="100%" fill="url(#s7aFormsGrid)" />
        <rect width="100%" height="100%" fill="url(#s7aFormsGridDiag)" />
      </svg>
    </div>
  );
}
...
export default function SBA7aFormsPage() {
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        .s7a-forms-page { min-height: 100vh; font-family: var(--font-body); background: var(--bg-primary); color: var(--text-primary); overflow-x: hidden; position: relative; }
        .s7a-forms-page::before { content: ''; position: fixed; inset: 0; background: radial-gradient(ellipse 60% 45% at 25% 10%, var(--overlay-gold), transparent), radial-gradient(ellipse 55% 40% at 75% 90%, var(--overlay-green), transparent); pointer-events: none; z-index: 0; }
        .s7a-forms-page a { color: inherit; }
        @media (max-width: 768px) { .s7a-forms-hero-title { font-size: 28px !important; } .s7a-forms-hero-inner { padding: 36px 16px 48px !important; } }
      `}</style>

      <div className="s7a-forms-page">
        <PremiumGrid />
        <PremiumParticles />
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ position: 'relative', overflow: 'hidden', background: 'var(--bg-hero)', borderBottom: '1px solid var(--border-gold)', animation: 'fadeSlideUp 0.7s ease-out both' }}>
            ...
            <div className="s7a-forms-hero-inner" style={{ maxWidth: 1200, margin: '0 auto', padding: '48px 32px 72px', position: 'relative', zIndex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 48 }}>
                <Link to="/access/sba-7a" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, color: '#c8e0b4', fontSize: 14, fontWeight: 600, textDecoration: 'none', fontFamily: 'var(--font-body)' }}>
              ...
              <h1 className="s7a-forms-hero-title" style={{ fontFamily: 'var(--font-display)', fontSize: 46, fontWeight: 400, textAlign: 'center', color: '#e8ede2', lineHeight: 1.15, letterSpacing: '-0.02em', marginBottom: 16 }}>
                SBA 7(a) <span style={{ fontStyle: 'italic', color: '#cda349' }}>Forms Library</span>
              </h1>
              ...
              <p style={{ textAlign: 'center', fontSize: 16, color: 'rgba(232,237,226,0.6)', lineHeight: 1.75, maxWidth: 620, margin: '0 auto', fontFamily: 'var(--font-body)' }}>
                All required forms and documents for your SBA 7(a) loan application, organized by category.
              </p>
            </div>
          </div>
          ...
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '48px 24px' }}>
            <FormsSection
              ...
              description="Core forms required for all SBA 7(a) loan applications"
            ...
            <FormsSection
              ...
              description="Additional documentation commonly required during the application process"
            ...
            <FormsSection
              ...
              description="Official SBA references and background information for the 7(a) program"
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.8, fontFamily: 'var(--font-body)' }}>
                    ...
                    <p>• Your lender may require additional documentation specific to your situation</p>
                    <p>• Some forms may be completed by the lender on your behalf</p>
          ...
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px 80px', textAlign: 'center' }}>
            <Link to="/access/sba-7a" style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '14px 32px', borderRadius: 14, background: 'linear-gradient(135deg, rgba(200,168,78,0.25), rgba(200,168,78,0.08))', border: '1px solid rgba(200,168,78,0.35)', color: 'var(--text-primary)', fontSize: 15, fontWeight: 600, fontFamily: 'var(--font-body)', textDecoration: 'none' }}>
              <ArrowLeft size={16} />Back to Toolkit Dashboard
            </Link>
          </div>
        </div>
