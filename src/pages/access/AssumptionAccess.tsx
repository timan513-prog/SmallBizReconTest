...
import { ArrowLeft, FileText, Download, Monitor, Package, CircleCheck as CheckCircle, FolderOpen, X, ExternalLink, ShieldCheck, Sparkles, Lock, ChevronRight, Crown, Zap, Award, Sun, Moon, Handshake } from 'lucide-react';
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, overflow: 'hidden', pointerEvents: 'none', zIndex: 0, opacity: 'var(--grid-opacity)' }}>
      <svg width="100%" height="100%">
        <defs>
          <pattern id="asmGrid" width="48" height="48" patternUnits="userSpaceOnUse">
            <path d="M 48 0 L 0 0 0 48" fill="none" stroke="var(--accent-gold)" strokeWidth="0.4" />
          </pattern>
          <pattern id="asmGridDiag" width="96" height="96" patternUnits="userSpaceOnUse">
            <path d="M 0 96 L 96 0" fill="none" stroke="var(--accent-gold)" strokeWidth="0.2" opacity="0.5" />
          </pattern>
        </defs>
        <rect width="100%" height="100%" fill="url(#asmGrid)" />
        <rect width="100%" height="100%" fill="url(#asmGridDiag)" />
      </svg>
    </div>
  );
}
...
const PremiumResourceCard: React.FC<{
  resource: ToolkitResource; index: number;
  onLaunchInteractive: (r: ToolkitResource) => void; onCloseModal: () => void;
}> = ({ resource, index, onLaunchInteractive, onCloseModal }) => {
  const [hovered, setHovered] = useState(false);
  const accent = resource.accentColor || accentForType(resource.type);

  const cardContent = (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} style={{ position: 'relative', overflow: 'hidden', background: 'var(--bg-card)', backdropFilter: 'var(--glass-blur)', WebkitBackdropFilter: 'var(--glass-blur)', border: `1px solid ${hovered ? 'var(--border-hover)' : 'var(--border-primary)'}`, borderRadius: 20, padding: 28, boxShadow: hovered ? 'var(--shadow-card-hover)' : 'var(--shadow-card)', transform: hovered ? 'translateY(-8px) scale(1.02)' : 'translateY(0) scale(1)', transition: 'all 0.6s cubic-bezier(0.23,1,0.32,1)', height: '100%', display: 'flex', flexDirection: 'column' as const, animation: `fadeSlideIn 0.5s ease-out ${0.1 + index * 0.08}s both`, cursor: 'pointer' }}>
      <div style={{ position: 'absolute', top: 0, left: 0, width: 4, height: '100%', background: `linear-gradient(180deg, ${accent}, ${accent}44)`, opacity: hovered ? 1 : 0.6 }} />
      ...
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 1, background: `linear-gradient(90deg, transparent, ${accent}44, transparent)`, opacity: hovered ? 1 : 0.3 }} />
      <div style={{ position: 'relative', zIndex: 1, flex: 1, display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <div style={{ width: 44, height: 44, borderRadius: 14, background: `${accent}15`, border: `1px solid ${accent}33`, display: 'flex', alignItems: 'center', justifyContent: 'center', ...(hovered ? { background: `${accent}25`, borderColor: `${accent}55` } : {}) }}>{resource.icon}</div>
          <div style={{ display: 'flex', gap: 6 }}>
            {resource.badge && <span style={{ fontSize: 10, fontWeight: 700, padding: '4px 10px', borderRadius: 8, background: 'var(--accent-gold-dim)', border: '1px solid var(--border-gold)', color: 'var(--accent-gold)', textTransform: 'uppercase', letterSpacing: '0.05em', fontFamily: 'var(--font-body)' }}>{resource.badge}</span>}
            <span style={{ fontSize: 10, fontWeight: 700, padding: '4px 10px', borderRadius: 8, background: `${accent}12`, border: `1px solid ${accent}30`, color: accent, textTransform: 'uppercase', letterSpacing: '0.05em', fontFamily: 'var(--font-body)' }}>{pillLabel(resource.type)}</span>
          </div>
        </div>
        ...
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontSize: 13, fontWeight: 600, color: accent, fontFamily: 'var(--font-body)', marginTop: 'auto', ...(hovered ? { gap: 12 } : {}) }}>
          ...
          <ChevronRight size={14} style={{ transform: hovered ? 'translateX(4px)' : 'translateX(0)', transition: 'transform 0.3s ease' }} />
        </div>
      </div>
    </div>
  );
...
const ToolkitContent: React.FC<ToolkitContentProps> = ({ onClose, startResourceId }) => {
  ...
  const toolkitData: ToolkitData = useMemo(() => ({
    title: 'COVID EIDL Loan Assumption Toolkit',
    description: 'Expert guidance for loan assumption requests',
    resources: [
      {
        id: 'asm-html',
        icon: <ExternalLink size={20} style={{ color: '#5d9ecf' }} />,
        title: 'Interactive Guide',
        description: 'Step-by-step interactive walkthrough for the loan assumption process',
        link: '#',
      ...
      },
      {
        ...
        title: 'Forms Library',
        description: 'All required forms for loan assumption submissions',
        link: '/access/assumption/forms',
      ...
      },
      {
        ...
        description: 'Professional templates for loan assumption requests',
        link: '/access/assumption/templates',
        type: 'internal-link' as ResourceType,
      ...
      <div style={{ position: 'relative', background: 'var(--bg-primary)', borderRadius: 24, maxWidth: 1100, width: '100%', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 32px 80px rgba(0,0,0,0.5), 0 0 0 1px var(--border-gold)', border: '1px solid var(--border-gold)', animation: 'fadeSlideIn 0.4s ease-out both' }}>
        <div style={{ position: 'relative', overflow: 'hidden', borderRadius: '24px 24px 0 0', background: 'var(--bg-hero)', borderBottom: '1px solid var(--border-gold)' }}>
          ...
          <div style={{ position: 'relative', padding: '32px 36px', color: '#e8ede2' }}>
            <button onClick={onClose} type="button" style={{ position: 'absolute', top: 16, right: 16, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 12, padding: 8, cursor: 'pointer', color: '#e8ede2', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><X size={20} /></button>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 12 }}>
              <div style={{ width: 48, height: 48, borderRadius: 16, background: 'rgba(200,168,78,0.15)', border: '1px solid rgba(200,168,78,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Crown size={24} color="#cda349" /></div>
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4, fontSize: 13, color: 'rgba(232,237,226,0.6)', fontFamily: 'var(--font-body)' }}><Lock size={13} />Lifetime access — re-download anytime</div>
              </div>
            </div>
          ...
          {interactiveResource ? (
            <div>
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: 20 }}>
                <button onClick={() => setInteractiveResource(null)} type="button" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'none', border: 'none', color: 'var(--accent-gold)', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--font-body)' }}><ArrowLeft size={16} />Back to Resources</button>
              </div>
              <div style={{ borderRadius: 20, overflow: 'hidden', border: '1px solid var(--border-primary)', boxShadow: 'var(--shadow-card)', padding: 40, textAlign: 'center' }}>
                ...
                <p style={{ fontSize: 14, color: 'var(--text-secondary)', fontFamily: 'var(--font-body)' }}>The interactive assumption guide is currently in development. Check back soon.</p>
              </div>
            </div>
          ) : (
            <>
...
const AssumptionAccess: React.FC = () => {
  ...
  useEffect(() => {
    setUnlockedToolkit(TOOLKIT_TYPES.ASSUMPTION);
  }, []);
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        .asm-page { min-height: 100vh; font-family: var(--font-body); background: var(--bg-primary); color: var(--text-primary); overflow-x: hidden; position: relative; transition: background 0.5s ease, color 0.4s ease; }
        .asm-page::before { content: ''; position: fixed; inset: 0; background: radial-gradient(ellipse 60% 45% at 25% 10%, var(--overlay-gold), transparent), radial-gradient(ellipse 55% 40% at 75% 90%, var(--overlay-green), transparent); pointer-events: none; z-index: 0; }
        .asm-page a { color: inherit; }
        @media (max-width: 768px) { .asm-hero-title { font-size: 30px !important; } .asm-hero-inner { padding: 36px 16px 48px !important; } .asm-cards-grid { grid-template-columns: 1fr !important; } }
      `}</style>
      ...
      <div className="asm-page">
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ position: 'relative', overflow: 'hidden', background: 'var(--bg-hero)', borderBottom: '1px solid var(--border-gold)', animation: 'fadeSlideUp 0.7s ease-out both' }}>
            ...
            <div className="asm-hero-inner" style={{ maxWidth: 1200, margin: '0 auto', padding: '48px 32px 72px', position: 'relative', zIndex: 1 }}>
              ...
              <div style={{ width: 76, height: 76, borderRadius: 22, background: 'linear-gradient(135deg, rgba(200,168,78,0.22), rgba(200,168,78,0.06))', border: '1px solid rgba(200,168,78,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 28px', boxShadow: '0 0 40px rgba(200,168,78,0.1)' }}>
                <Handshake size={36} color="#cda349" strokeWidth={1.5} />
              </div>

              <div style={{ textAlign: 'center', marginBottom: 16 }}>
                <span style={{ padding: '5px 16px', borderRadius: 100, background: 'rgba(200,168,78,0.12)', border: '1px solid rgba(200,168,78,0.25)', fontSize: 11, fontWeight: 700, color: '#cda349', fontFamily: 'var(--font-body)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>★ Premium Toolkit</span>
              </div>

              <h1 className="asm-hero-title" style={{ fontFamily: 'var(--font-display)', fontSize: 48, fontWeight: 400, textAlign: 'center', color: '#e8ede2', lineHeight: 1.15, letterSpacing: '-0.02em', marginBottom: 16 }}>
                COVID EIDL Loan{' '}
                <span style={{ fontStyle: 'italic', color: '#cda349' }}>Assumption Toolkit</span>
              </h1>
              ...
              <p style={{ textAlign: 'center', fontSize: 16, color: 'rgba(232,237,226,0.6)', lineHeight: 1.75, maxWidth: 640, margin: '0 auto 28px', fontFamily: 'var(--font-body)' }}>
                Your complete DIY guide for navigating the SBA COVID EIDL loan assumption process with confidence. All materials included with lifetime access.
              </p>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'center' }}>
                ...
                <button type="button" onClick={() => openModal('asm-html')} style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '16px 36px', borderRadius: 16, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', color: '#e8ede2', fontSize: 16, fontWeight: 600, fontFamily: 'var(--font-body)', cursor: 'pointer' }}>
            ...
            <div style={{ position: 'relative', overflow: 'hidden', background: 'var(--bg-card)', backdropFilter: 'var(--glass-blur)', WebkitBackdropFilter: 'var(--glass-blur)', borderRadius: 24, border: '1px solid var(--border-gold)', padding: 40, boxShadow: 'var(--shadow-gold)', animation: 'fadeSlideUp 0.7s ease-out 0.15s both', textAlign: 'center' }}>
              ...
              <div style={{ position: 'relative', zIndex: 1 }}>
                ...
                <p style={{ fontSize: 16, color: 'var(--text-secondary)', lineHeight: 1.7, maxWidth: 600, margin: '0 auto 24px', fontFamily: 'var(--font-body)' }}>
                  Welcome to your complete loan assumption toolkit. All materials are included with your purchase and available for lifetime access.
                </p>
                <div style={{ maxWidth: 560, margin: '0 auto', padding: '14px 20px', borderRadius: 14, background: 'var(--accent-gold-dim)', border: '1px solid var(--border-gold)' }}>
                  <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)', fontFamily: 'var(--font-body)' }}>
                    🤖 Sabbi 2.0 is unlocked — tap the Sabbi button in the bottom right to ask loan assumption questions.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 24px' }}>
            <div className="asm-cards-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24, alignItems: 'stretch' }}>
              <AccessCard index={0} title="Interactive Guide" description="Step-by-step interactive HTML toolkit for the loan assumption process. Coming soon." accentColor="#5d9ecf" iconBg="rgba(93,158,207,0.12)" iconBorder="rgba(93,158,207,0.3)" icon={<Monitor size={26} style={{ color: '#5d9ecf' }} />} badge="Coming Soon"
                actionElement={<PremiumButton onClick={() => openModal('asm-html')} accentColor="#5d9ecf" icon={<Zap size={16} />}>Coming Soon</PremiumButton>}
              />
              <AccessCard index={1} title="PDF Pro Version" description="The full professional-grade PDF guide for loan assumption — comprehensive, print-ready, and annotated for DIY filers." accentColor="#cf5d5d" iconBg="rgba(207,93,93,0.12)" iconBorder="rgba(207,93,93,0.3)" icon={<Download size={26} style={{ color: '#cf5d5d' }} />} badge="Coming Soon"
                actionElement={<PremiumButton href="#" accentColor="#cf5d5d" icon={<Download size={16} />}>Coming Soon</PremiumButton>}
              />
              <AccessCard index={2} title="PDF Version" description="Standard PDF guide for the loan assumption process — clear, concise, and ready to follow at your own pace." accentColor="#5daa5d" iconBg="rgba(93,170,93,0.12)" iconBorder="rgba(93,170,93,0.3)" icon={<FileText size={26} style={{ color: '#5daa5d' }} />} badge="Coming Soon"
                actionElement={<PremiumButton href="#" accentColor="#5daa5d" icon={<FileText size={16} />}>Coming Soon</PremiumButton>}
              />
              <AccessCard index={3} title="Templates & Docs" description="Sample letters, forms, and professional templates for your loan assumption requests." accentColor="#9e7ec8" iconBg="rgba(158,126,200,0.12)" iconBorder="rgba(158,126,200,0.3)" icon={<FileText size={26} style={{ color: '#9e7ec8' }} />}
                actionElement={<PremiumButton to="/access/assumption/templates" accentColor="#9e7ec8" variant="outline" icon={<FileText size={16} />}>View Templates</PremiumButton>}
              />
              <AccessCard index={4} title="Advanced Forms Library" description="View all required loan assumption forms categorized by usage — ready-to-use and professionally formatted." accentColor="#e0943d" iconBg="rgba(224,148,61,0.12)" iconBorder="rgba(224,148,61,0.3)" icon={<FolderOpen size={26} style={{ color: '#e0943d' }} />}
                actionElement={<PremiumButton to="/access/assumption/forms" accentColor="#e0943d" icon={<ChevronRight size={16} />}>View Forms</PremiumButton>}
              />
            </div>
          </div>
            ...
            <div style={{ position: 'relative', overflow: 'hidden', background: 'var(--bg-card)', backdropFilter: 'var(--glass-blur)', WebkitBackdropFilter: 'var(--glass-blur)', borderRadius: 24, border: '1px solid var(--border-primary)', padding: 48, boxShadow: 'var(--shadow-card)', textAlign: 'center', animation: 'fadeSlideUp 0.7s ease-out 0.6s both' }}>
              ...
              <div style={{ position: 'relative', zIndex: 1 }}>
                ...
                <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 28, fontWeight: 400, color: 'var(--text-primary)', marginBottom: 12 }}>
                  Loan Assumption <span style={{ fontStyle: 'italic', color: 'var(--accent-gold)' }}>Interactive Toolkit</span>
                </h2>
                <p style={{ fontSize: 15, color: 'var(--text-secondary)', lineHeight: 1.7, maxWidth: 500, margin: '0 auto 28px', fontFamily: 'var(--font-body)' }}>
                  The interactive step-by-step guide for the loan assumption process is currently in development. Check back soon for the full experience.
                </p>
...
export default AssumptionAccess;
