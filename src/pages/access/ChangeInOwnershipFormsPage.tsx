  ...
  return (
    <div style={{ position: 'fixed', inset: 0, overflow: 'hidden', pointerEvents: 'none', zIndex: 0, opacity: 'var(--grid-opacity)' }}>
      <svg width="100%" height="100%">
        <defs>
          <pattern id="cioFormsGrid" width="48" height="48" patternUnits="userSpaceOnUse">
            <path d="M 48 0 L 0 0 0 48" fill="none" stroke="var(--accent-gold)" strokeWidth="0.4" />
          </pattern>
        </defs>
        <rect width="100%" height="100%" fill="url(#cioFormsGrid)" />
      </svg>
    </div>
  );
}
...
const PremiumFormCard: React.FC<{ form: FormItem; index: number; sectionAccent: string }> = ({ form, index, sectionAccent }) => {
  ...
  const buttonStyle = (variant: 'filled' | 'outline', disabled: boolean): React.CSSProperties => ({
    ...
    ...(variant === 'filled'
      ? { background: `linear-gradient(135deg, ${accent}55, ${accent}25)`, border: `1px solid ${accent}77`, color: 'var(--text-primary)' }
    ...
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} style={{ position: 'relative', overflow: 'hidden', background: 'var(--bg-card)', backdropFilter: 'var(--glass-blur)', WebkitBackdropFilter: 'var(--glass-blur)', border: `1px solid ${hovered ? 'var(--border-hover)' : 'var(--border-primary)'}`, borderRadius: 20, padding: 24, boxShadow: hovered ? 'var(--shadow-card-hover)' : 'var(--shadow-card)', transform: hovered ? 'translateY(-6px) scale(1.015)' : 'translateY(0) scale(1)', transition: 'all 0.6s cubic-bezier(0.23,1,0.32,1)', display: 'flex', flexDirection: 'column', height: '100%', animation: `fadeSlideIn 0.5s ease-out ${0.05 + index * 0.04}s both` }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 'auto' }}>
          {form.viewUrl && (
            <a href={form.viewUrl} target="_blank" rel="noopener noreferrer" style={buttonStyle('outline', isComingSoon)} aria-disabled={isComingSoon} tabIndex={isComingSoon ? -1 : 0}>
              <ExternalLink size={15} />{form.customButtonText || 'View on SBA.gov'}
            </a>
          )}
          {form.downloadPath && (
            <a href={form.downloadPath} target="_blank" rel="noopener noreferrer" download={!isComingSoon} style={buttonStyle('filled', isComingSoon)} aria-disabled={isComingSoon} tabIndex={isComingSoon ? -1 : 0}>
              <Download size={15} />{form.downloadButtonText || 'Download File'}
            </a>
          )}
...
const FormsSection: React.FC<{
  ...
  <div id={id} className="scroll-mt-24" style={{ background: 'var(--bg-card)', backdropFilter: 'var(--glass-blur)', WebkitBackdropFilter: 'var(--glass-blur)', borderRadius: 20, border: '1px solid var(--border-primary)', overflow: 'hidden', boxShadow: 'var(--shadow-card)' }}>
    <button onClick={onToggle} type="button" style={{ width: '100%', padding: '24px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'transparent', border: 'none', borderBottom: isOpen ? '1px solid var(--border-primary)' : 'none', cursor: 'pointer', transition: 'all 0.3s ease' }}>
...
const requiredForms: FormItem[] = [
  { title: 'COVID EIDL Change in Ownership Requirements', description: 'Official SBA requirements and checklist for change in ownership requests', viewUrl: 'https://www.sba.gov/document/support-covid-eidl-change-ownership', accentColor: '#D4B483' },
  ...
  { title: 'SBA Form 1919 -- Borrower Information Form', description: 'Required for ownership changes. Collects information about the new ownership structure.', viewUrl: 'https://www.sba.gov/document/sba-form-1919-borrower-information-form', downloadPath: 'https://www.sba.gov/document/sba-form-1919-borrower-information-form', accentColor: '#D4B483' },
  { title: 'SBA Form 413 -- Personal Financial Statement', description: 'Required for new owners with 20%+ ownership. Documents personal financial condition.', viewUrl: 'https://www.sba.gov/document/sba-form-413-personal-financial-statement', downloadPath: 'https://rufgrcaxemvewhqiayfk.supabase.co/storage/v1/object/public/sbaforms//SBAForm413%20PERSONAL%20FINANCIAL%20STATEMENT.pdf', accentColor: '#D4B483' },
  { title: 'Business Purchase Agreement', description: 'Legal agreement documenting the sale and purchase of the business — required to verify ownership transfer terms', customDate: '*Coming Soon*', accentColor: '#D4B483' },
  { title: 'Articles of Organization / Incorporation', description: 'Organizational documents showing the new ownership structure of the business entity', customDate: '*Coming Soon*', accentColor: '#D4B483' },
];
...
const optionalForms: FormItem[] = [
  ...
  { title: 'YTD Profit and Loss Statement', description: 'Most recent YTD Profit and Loss Statement of the assuming/new party', viewUrl: 'https://rufgrcaxemvewhqiayfk.supabase.co/storage/v1/object/public/sbaforms//SBARecon_YTD_Profit_and_Loss_Statement_Template.xlsx', downloadPath: 'https://rufgrcaxemvewhqiayfk.supabase.co/storage/v1/object/public/sbaforms//SBARecon_YTD_Profit_and_Loss_Statement_Template.xlsx' },
  { title: 'IRS Form 4506-C -- Tax Transcript Request', description: 'Authorizes the IRS to provide tax return transcripts for new ownership review', viewUrl: 'https://www.irs.gov/forms-pubs/about-form-4506-t', downloadPath: 'https://www.irs.gov/forms-pubs/about-form-4506-t', accentColor: '#3B4C1C' },
];
...
const ChangeInOwnershipFormsPage: React.FC = () => {
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        .cio-forms-page { min-height: 100vh; font-family: var(--font-body); background: var(--bg-primary); color: var(--text-primary); overflow-x: hidden; position: relative; transition: background 0.5s ease, color 0.4s ease; }
        .cio-forms-page::before { content: ''; position: fixed; inset: 0; background: radial-gradient(ellipse 60% 45% at 25% 10%, var(--overlay-gold), transparent), radial-gradient(ellipse 55% 40% at 75% 90%, var(--overlay-green), transparent); pointer-events: none; z-index: 0; }
        .cio-forms-page a { color: inherit; }
        @media (max-width: 768px) { .cio-forms-hero-title { font-size: 28px !important; } .cio-forms-hero-inner { padding: 36px 16px 48px !important; } }
      `}</style>
      <div className="cio-forms-page">
        <PremiumGrid />
        <PremiumParticles />
        <div style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ position: 'relative', overflow: 'hidden', background: 'var(--bg-hero)', borderBottom: '1px solid var(--border-gold)', animation: 'fadeSlideUp 0.7s ease-out both' }}>
            ...
            <div className="cio-forms-hero-inner" style={{ maxWidth: 1200, margin: '0 auto', padding: '48px 32px 72px', position: 'relative', zIndex: 1 }}>
              ...
              <h1 className="cio-forms-hero-title" style={{ fontFamily: 'var(--font-display)', fontSize: 46, fontWeight: 400, textAlign: 'center', color: '#e8ede2', lineHeight: 1.15, letterSpacing: '-0.02em', marginBottom: 16 }}>
                Change in Ownership <span style={{ fontStyle: 'italic', color: '#cda349' }}>Forms Library</span>
              </h1>
              <p style={{ textAlign: 'center', fontSize: 16, color: 'rgba(232,237,226,0.6)', lineHeight: 1.75, maxWidth: 620, margin: '0 auto 28px', fontFamily: 'var(--font-body)' }}>
                Premium change in ownership forms categorized by usage. All forms are included with your toolkit access.
              </p>
          ...
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 24px 32px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              <div style={{ animation: 'fadeSlideUp 0.7s ease-out 0.25s both' }}>
                <FormsSection id="required" title="Required Forms" subtitle="These forms are mandatory for change in ownership submission. Missing any of these will result in delays or rejection." emoji="🔴" forms={requiredForms} sectionAccent="#cf6b6b" isOpen={openSections.has('required')} onToggle={() => toggleSection('required')} />
              </div>
              ...
              <div style={{ animation: 'fadeSlideUp 0.7s ease-out 0.35s both' }}>
                <FormsSection id="informational" title="Informational" subtitle="General references and background information to help you understand the change in ownership process." emoji="🔵" forms={informationalForms} sectionAccent="#6b9fcf" isOpen={openSections.has('informational')} onToggle={() => toggleSection('informational')} />
              </div>
            </div>
          </div>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.8, fontFamily: 'var(--font-body)' }}>
                    ...
                    <p>• For change in ownership requests, complete and accurate documentation of the new ownership structure is critical</p>
                  </div>
                </div>
              </div>
            </div>
...
export default ChangeInOwnershipFormsPage;
